...
class BaseTextCategorizationDataset:
    ...
    def _get_num_train_samples(self):
        ...
        (training set size)
        """
        return int(self._get_num_samples() * self.train_ratio)
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        def filter_tag_position(position):
            def filter_function(df):
                ...
                """
                dataset = dataset[dataset['tag_position'] == 0]
        ...
        counts = dataset['tag_name'].value_counts()
        frequent_tags = counts[counts >= min_samples_per_label].index.tolist()
        dataset = dataset[dataset['tag_name'].isin(frequent_tags)]
